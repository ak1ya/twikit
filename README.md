...
```bash
# ツイートする
python main.py tweet "ツイート本文"

# アカウントを指定してツイートする
python main.py --account sub tweet "サブアカウントのツイート"

# メディア付きツイート
python main.py tweet "ツイート本文" --media image1.png image2.png

# ツイートを検索する
python main.py search-tweet "検索クエリ"
python main.py search-tweet "検索クエリ" --type Latest

# ユーザーを検索する
python main.py search-user "検索クエリ"

# トレンドを取得する
python main.py trends
python main.py trends --category sports
...
python main.py logs --account main
...
ツイート本文に `{{変数名}}` を埋め込むと、投稿時の日時に自動展開されます。
...
```bash
python main.py tweet "{{date}} {{weekday}}、おはようございます！"
# → "2026-05-17 日曜日、おはようございます！"
```

スケジューラーでは**登録時ではなく実際の投稿時**に展開されるため、毎日正しい日付が入ります。
